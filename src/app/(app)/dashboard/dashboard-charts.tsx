...
const TABUNG_COLORS = { '3kg': '#3b82f6', '5kg': '#f59e0b', '12kg': '#a855f7' };

function KPICard({ label, value, sub, icon, accent }: { label: string; value: string; sub?: string; icon: string; accent?: string }) {
  return (
    <div className={`rounded-xl border bg-card p-4 shadow-sm transition-shadow hover:shadow-md ${accent ? `border-l-4 ${accent}` : ''}`}>
  ...
  return (
    <div className={`space-y-6 ${isPending ? 'opacity-60 pointer-events-none' : ''}`}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard label="Total Outstanding" value={formatRupiah(summary.totalOutstanding)} icon="💰" accent="border-l-blue-500" />
        <KPICard label="Jatuh Tempo" value={`${summary.overdueCount} invoice`} sub={summary.overdueTotal > 0 ? formatRupiah(summary.overdueTotal) : undefined} icon="⚠️" accent={summary.overdueCount > 0 ? 'border-l-red-500' : undefined} />
        <KPICard label="Total Ditagih" value={formatRupiah(summary.totalInvoiced)} sub={`${summary.invoiceCount} invoice`} icon="📄" accent="border-l-emerald-500" />
        <KPICard label="Total Dibayar" value={formatRupiah(summary.totalPaid)} icon="✅" accent="border-l-green-500" />
      </div>

      <div className="grid gap-4 sm:grid-cols-4">
        <KPICard label="Total Tabung Dikirim" value={String(summary.totalTabung)} icon="📦" />
        <KPICard label="Tabung 3kg" value={String(summary.tabung3kg)} icon="🔵" accent="border-l-blue-500" />
        <KPICard label="Tabung 5kg" value={String(summary.tabung5kg)} icon="🟡" accent="border-l-amber-500" />
        <KPICard label="Tabung 12kg" value={String(summary.tabung12kg)} icon="🟣" accent="border-l-purple-500" />
      </div>
          ...
          <div className="rounded-xl border bg-card p-4 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={monthly} barGap={4}>
                ...
                <Legend />
                <Bar dataKey="invoiced" name="Ditagih" fill="#3b82f6" radius={[4, 4, 0, 0]} />
              ...
              <div className="space-y-2 max-h-[250px] overflow-y-auto">
                {overdueList.map((inv: any) => (
                  <div key={inv.id} className="flex items-center justify-between rounded-lg border border-destructive/20 bg-destructive/5 p-3">
                    <div><p className="text-sm font-medium">{inv.nomor_invoice}</p><p className="text-xs text-muted-foreground">{(inv.pangkalan as any)?.nama}</p></div>
            ...
            {pangkalanRanking.length > 0 ? (
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={pangkalanRanking} layout="vertical" barSize={16}>
                  ...
                  <Tooltip content={<ChartTooltip />} />
                  <Legend />
                  <Bar dataKey="totalInvoiced" name="Ditagih" fill="#3b82f6" radius={[0, 4, 4, 0]} />
                  <Bar dataKey="totalPaid" name="Dibayar" fill="#22c55e" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
                ...
                return (
                  <div key={p.nama} className="rounded-lg border p-3">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <div className="flex-1 rounded-full bg-muted h-2"><div className="h-full rounded-full bg-blue-500" style={{ width: `${pct}%` }} /></div>
                      <span className="text-xs text-muted-foreground tabular-nums">{pct.toFixed(1)}% terkumpul</span>
                    </div>
                    <p className="mt-1 text-xs text-green-600">Dibayar: {formatRupiah(p.totalPaid)}</p>
                  </div>
                );
              })}
            </div>
          ...
          <div className="space-y-2">
            {recentPayments.map((p: any) => (
              <div key={p.id} className="flex items-center justify-between rounded-lg border p-3 transition-colors hover:bg-muted/30">
                ...
                <p className="text-sm font-semibold tabular-nums text-green-600">+{formatRupiah(p.nominal)}</p>
              </div>
            ))}
          </div>
